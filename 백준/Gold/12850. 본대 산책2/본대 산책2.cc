...
int main()
{
	...
	matrix CurrMatrix = GetMap(); //진행되면서 계속 제곱될 예정
	result = IdentityMatrix();
	
	vector<int> binArr = ToBinary(d);
	...
	{
		if(binArr[i] == 1)
			result = MatMul(CurrMatrix, result, 8);

		CurrMatrix = MatMul(CurrMatrix, CurrMatrix, 8);
	}
