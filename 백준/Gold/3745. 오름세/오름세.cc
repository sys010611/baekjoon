...
int main()
{
	...
	while (cin >> _n)
	{
		...
		int input;
		for (int i = 1; i <= _n; i++) // 편의상 인덱스는 1부터 시작
		{
			cin >> input;
		...
		for (auto it = v.begin(); it != v.end(); it++)
		{
			...
			segmentTree[it->second + N - 1] = GetMaxLength(1, it->second, 1, N, 1) + 1; // 최대길이 저장
			// +N - 1: 수열의 인덱스를 트리의 인덱스로 변환한 것
			UpdateTree(it->second + N - 1);

			//PrintTree();
		}

		//printf("\n\n\n\n%d부터 %d까지 범위 내 최장 길이: ", 1, N);
		cout << GetMaxLength(1, _n, 1, N, 1) << "\n";
	}
